...
fetch('./data.json')
    ...
    .then(personalInfo => {
        ...
        document.getElementById("team-github").parentElement.innerHTML = `<a href="${personalInfo.teamGithub}" target="_blank"><i class="fa fa-github" aria-hidden="true">&nbsp;&nbsp;</i>Team GitHub</a>`;
        document.getElementById("github").parentElement.innerHTML = `<a href="${personalInfo.github}" target="_blank"><i class="fa fa-github" aria-hidden="true">&nbsp;&nbsp;</i>Personal</a>`;
        ...
        document.querySelector(".contactDetails .fa-phone").parentElement.innerHTML = `<a href="tel:+91${personalInfo.phone}"><i class="fa fa-phone" aria-hidden="true">&nbsp;&nbsp;</i>+91 ${personalInfo.phone}</a>`;
        ...
        personalInfo.experience.forEach((exp, index) => {
            const li = document.createElement("li");
            li.innerHTML = `<span class="subDetails">${exp.title} &nbsp;</span><span class="bold"></span>- ${exp.company}, ${exp.location} <br> ${exp.duration} <br>${exp.highlights}${index < personalInfo.experience.length - 1 ? '<br><br>' : ''}`;
        ...
        personalInfo.projects.forEach((project, index) => {
            ...
            let projectLink = '';
            if (project.link) {
                projectLink += `<a href="${project.link}" target="_blank">Live Demo</a> &nbsp;&nbsp;`;
            }
            if (project.github) {
            ...
            let projectDetails = `<span class="subDetails">${project.title} &nbsp;</span><span class="bold"></span> [<span class="projectTech">${project.technologies.join(', ')}</span>] <br> <span class="projectHighlights">${project.description}</span> <br>`;
            if (project.duration) {
